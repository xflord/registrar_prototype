...
public interface FormItemJdbcCrudRepository extends CrudRepository<FormItemEntity, Integer> {
  ...
  @Query("SELECT fi.* FROM form_item fi " +
           "JOIN item_definition_form_types ft ON ft.item_definition_id = fi.item_definition_id " +
           "WHERE fi.form_id = :formSpecificationId AND ft.form_type = :formType")
//  @Query("""
//           SELECT fi.* FROM "form_item" fi
//           JOIN "item_definition_form_types" ft ON ft.item_definition_id = fi.item_definition_id
//           WHERE fi.form_id = :formSpecificationId AND ft.form_type = :formType
//           """)
    List<FormItemEntity> findByFormSpecificationIdAndFormType(
        @Param("formSpecificationId") int formSpecificationId,
        @Param("formType") FormSpecification.FormType formType);
}
