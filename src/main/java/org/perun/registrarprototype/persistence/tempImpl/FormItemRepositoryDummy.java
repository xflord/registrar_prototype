...
public class FormItemRepositoryDummy implements FormItemRepository {
  ...
  public List<FormItem> getFormItemsByFormIdAndType(int formId, FormSpecification.FormType formType) {
    return getFormItemsByFormId(formId).stream().filter(item -> itemDefinitionRepository.findById(item.getItemDefinitionId()).get().getFormTypes().contains(formType)).toList();
  }
