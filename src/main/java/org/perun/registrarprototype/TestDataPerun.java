...
public class TestDataPerun {
  ...
  CommandLineRunner initDatabase(FormService formService, ApplicationService applicationService) {
    return args -> {
      ...
      // Get the actual form ID after creation
      int formId = formSpecification.getId();
      ...
      PrefillStrategyEntry prefillStrat = new PrefillStrategyEntry(-1,
          PrefillStrategyEntry.PrefillStrategyType.IDM_ATTRIBUTE, new HashMap<>(Map.of("test", "test")), ADDRESS_ATTR_DEF_U, formId, false);
      prefillStrat = formService.createPrefillStrategy(prefillStrat);
      ItemDefinition itemDef1 = new ItemDefinition(-1, formId, "address", ItemType.TEXTFIELD, true, false, null,
      ...
      FormItem item1 = new FormItem(-1, formId, "address", null, 1, null, null, itemDef1.getId());
      ...
      destination = formService.createDestination(destination);
      PrefillStrategyEntry prefillStrat2 = new PrefillStrategyEntry(-2, PrefillStrategyEntry.PrefillStrategyType.IDENTITY_ATTRIBUTE, new HashMap<>(), "name", formId, false);
      prefillStrat2 = formService.createPrefillStrategy(prefillStrat2);
      ItemDefinition itemDef2 = new ItemDefinition(-2, formId, "full name", ItemType.TEXTFIELD, false, true, null,
      ...
      FormItem item2 = new FormItem(-2, formId, "full name", null, 2, null, null, itemDef2.getId());

      ItemDefinition itemDef3 = new ItemDefinition(-3, formId, "submit", ItemType.SUBMIT_BUTTON, false, false, null,
          null, null, Set.of(FormSpecification.FormType.INITIAL),
          Map.of(Locale.ENGLISH, itemTexts2), ItemDefinition.Condition.NEVER,
          ItemDefinition.Condition.NEVER, null, false);
      itemDef3 = formService.createItemDefinition(itemDef3);
      FormItem submit = new FormItem(-3, formId, "submit", null, 3, null, null, itemDef3.getId());

      formService.updateFormItems(formId, List.of(item1, item2, submit));
