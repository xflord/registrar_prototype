...
public class IdmAttributePrefillStrategy implements PrefillStrategy {
  ...
  public Optional<String> prefill(FormItem item, Map<String, String> options) {
    ...
    } else if (sourceAttr.startsWith(idmService.getMemberAttributeUrn()) && formSpecification.getGroupId() != null) { // TODO can get member attr just from voId
      return Optional.ofNullable(
          idmService.getMemberAttribute(sessionProvider.getCurrentSession().getPrincipal().id(), sourceAttr, formSpecification.getGroupId()));
    } else if (sourceAttr.startsWith(idmService.getGroupAttributeUrn()) && formSpecification.getGroupId() != null) { // TODO better check if group is present
